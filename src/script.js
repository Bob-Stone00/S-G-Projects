...
import './TrackAndTracingOrderTracking2.css';
...
const locations = [
  { title: "Out of Factory", city: "China" },
  { title: "On Transit", city: "OR TAMBO, SA" },
  { title: "Delivered", city: "Pretoria, SA" },
];
...
const TrackAndTracingOrderTracking2 = () => {
  ...
  const displayProductJourney = (productId) => {
    ...
    let journey = productBlocks.map(block => {
      const location = getRandomLocation();
      const timestamp = new Date(block.timestamp).toLocaleString();
      return `
        <li>
          <span class="journey-timestamp">${timestamp}</span>
          - ${block.data.name} (Description: ${block.data.description})
...
const ProductJourneyModal = ({ journey, journeyProductId, closeModal }) => {
  return (
    <div id="product-journey-modal" className="product-journey-modal">
      ...
      <ul id="journeyListElement" dangerouslySetInnerHTML={{ __html: journey }}></ul>
